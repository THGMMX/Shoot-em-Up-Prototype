...
function Star(x, y) {
  let r = Math.max(50,Math.floor(Math.random()*256));
  let g = Math.max(125, Math.floor(Math.random()*256));
  let b = Math.max(240, Math.floor(Math.random()*256));
  this.x = Math.floor(x);
  this.y = Math.floor(y);
  let size = Math.floor(Math.random() * Star.MAX_SIZE);
  this.width = size;
  this.height = size;
  this.letiance = Math.max(Math.random(), .2);
...
Star.prototype.update = function() {
  if (game.overdrive) {
    let travel = Star.SPEED*this.letiance*6;
    this.width = 50;
  } else {
    let travel = Star.SPEED*this.letiance;
    ...
    game.currentState.enemyList.forEach(function(enemy) {
      ...
      if (Intersection.rectanglesIntersect(bullet, enemy)) {
        ...
        if (!enemy.alive) {
          ...
          game.updateScore();
          enemy.hit = true;
          let explosion = game.currentState.explosions.find(notAlive);
          explosion.x = enemy.x;
          explosion.y = enemy.y;
...
Player.prototype.shoot = function() {
  ...
  if (this.powerLevel === 1) {
    let bullet = this.bulletPool.find(notAlive);
    if (!bullet) {
      return;
    }
  ...
  if (this.powerLevel === 2) {
    ...
    let bullet2 = this.bulletPool.find(notAlive);
    if (!bullet) {
      return;
    }
    bullet2.alive = true;
  ...
  if (this.powerLevel >= 3) {
    ...
    let bullet3 = this.bulletPool.find(notAlive);
    if (!bullet3) {
      return;
    }
    bullet3.alive = true;
